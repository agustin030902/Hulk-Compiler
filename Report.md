...
- **Arrays y vectores**: Soporte para arrays unidimensionales y multidimensionales (`T[]`, `T[][]`), literales `{1,2,3}`, construcción con inicialización lambda (`new T[n]{ i -> expr }`), acceso por índice y método `.size()`.
...
Una de las áreas más interesantes del análisis semántico es la verificación del bucle `for`. El compilador debe determinar si el tipo sobre el que se itera es válido para la iteración, y además **validar que las firmas de los métodos del protocolo sean correctas**.

El algoritmo en `check_for_expr` (for_expr_checker.rs) resuelve el tipo del elemento siguiendo una estrategia en dos pasos con validación de firmas:

1. **Busca `current()` directamente**: Si el tipo tiene un método `current()`, es un tipo `Iterable` directo (como `Range`). El compilador entonces valida que también exista `next()` con firma correcta: `next(): Boolean` (sin parámetros). Si `next()` no existe o tiene firma incompatible, se reporta un error específico. El tipo del elemento es el tipo de retorno de `current()`.
2. **Busca `iter()` como fallback**: Si el tipo no tiene `current()`, busca un método `iter()` que retorne un `Iterable` (sin parámetros). Si lo encuentra, verifica que el tipo retornado por `iter()` tenga `next(): Boolean` y `current(): T` con las firmas correctas. El tipo del elemento es el tipo de retorno de `current()` en el iterador retornado.
3. **Error**: Si no se encuentra ninguno de los dos protocolos, se reporta: "Type X is not iterable or enumerable."

**Validación de firmas**: El compilador no solo busca métodos por nombre, sino que verifica sus firmas completas:

- `next()` debe tener exactamente 0 parámetros y retornar `Boolean`.
- `current()` debe tener exactamente 0 parámetros y retornar un tipo válido.
- Para `Enumerable`, `iter()` debe tener exactamente 0 parámetros y retornar un tipo que tenga `next(): Boolean` y `current(): T`.

**Búsqueda de métodos heredados**: Los métodos pueden estar definidos en tipos padre. Por ejemplo, `Iterable_Number` hereda `next()` de `Iterable`. El compilador usa `find_method_in_parent` para recorrer la cadena de herencia y encontrar métodos que no están definidos directamente en el tipo. Esto permite que un tipo que solo define `current()` satisfaga el protocolo `Iterable` si hereda `next()` de un padre.

**Mensajes de error específicos**: Cuando un tipo no cumple un protocolo, el error identifica exactamente qué falta:

- `"Type X is not iterable or enumerable. It must implement 'current()' (Iterable) or 'iter()' (Enumerable)."`
- `"Type X must implement 'next()' returning Boolean for Iterable protocol (inherited from parent)."`
- `"Method 'next' must return Boolean, found type X."`
- `"Method 'iter' in type X must return an Iterable, found type Y."`

Esta estrategia en cascada con validación de firmas es clave porque permite que dos tipos de objetos funcionen en el `for`:

- **Tipos `Iterable`**: Tienen `next()` y `current()` directamente (o heredados). El tipo en sí es el iterador.
...
- **`Iterable`**: Un tipo que **es** su propio iterador. Tiene `next(): Boolean` y `current(): T` directamente (o heredados de un padre). El tipo y el iterador son la misma cosa. Ejemplo: `Range`.
- **`Enumerable`**: Un tipo que **puede crear** iteradores independientes. Tiene `iter(): Iterable` que retorna un objeto iterador separado. El iterador retornado debe tener `next(): Boolean` y `current(): T`. Cada llamada a `iter()` crea un nuevo iterador con su propio estado. Ejemplo: una lista que puede ser recorrida múltiples veces.

**Validación de firmas**: El compilador no solo verifica que los métodos existan, sino que sus firmas sean correctas:

- `next()` debe tener 0 parámetros y retornar `Boolean`.
- `current()` debe tener 0 parámetros y retornar un tipo válido.
- `iter()` debe tener 0 parámetros y retornar un tipo que a su vez tenga `next(): Boolean` y `current(): T`.

**Búsqueda en la cadena de herencia**: Los métodos pueden estar heredados de tipos padre. Por ejemplo, `Iterable_Number` define `current(): Number` pero hereda `next(): Boolean` de `Iterable`. El compilador recorre la cadena de herencia para verificar que todos los métodos requeridos estén disponibles, ya sea definidos directamente o heredados.
...
### 7.7 Arrays y Vectores

Los arrays (vectores) son una estructura de datos fundamental que permite almacenar colecciones homogéneas de elementos. HULK soporta arrays unidimensionales y multidimensionales con una sintaxis inspirada en lenguajes como C#, Java y TypeScript.

#### 7.7.1 Tipo `Array` y notación `T[]`

El tipo `Array` es un built-in registrado en el sistema de tipos con `TypeTable::array` (TypeID 10). A diferencia de otros builtins, `Array` no tiene campos, métodos ni constructor registrados — se comporta como un tipo opaco.

La notación de tipo para arrays usa corchetes:

| Sintaxis | Tipo semántico | Descripción |
|----------|---------------|-------------|
| `Number[]` | `Array(Number)` | Array unidimensional de números |
| `Boolean[]` | `Array(Boolean)` | Array unidimensional de booleanos |
| `Number[][]` | `Array(Array(Number))` | Array bidimensional de números |
| `String[][][]` | `Array(Array(Array(String)))` | Array tridimensional de strings |
| `Point[]` | `Array(Point)` | Array de objetos Point |

El parser resuelve la notación `T[]` como una anotación de tipo con `is_vector: true` y `vector_dims` contando el número de niveles de corchetes. El SemanticType correspondiente es `SemanticType::Array(Box::new(element_type))`, anidado recursivamente para cada dimensión.

**¿Por qué no un genérico `Array<T>`?** Porque HULK no tiene genéricos. La notación `T[]` es una forma limitada de parametrización que el compilador resuelve completamente en tiempo de compilación, sin generar código polymórfico.

#### 7.7.2 Literales de array

Los arrays se pueden crear con literales usando llaves:
...
**Regla del parser**: Un literal de array requiere **al menos dos elementos** separados por coma: `{expr1, expr2, ...}`. Esto evita ambigüedad con expresiones de bloque (`{ stmt1; stmt2; }`). Como consecuencia, `{}` es un bloque vacío (no un array vacío) y `{x}` es una expresión de bloque (no un array de un elemento).

**Verificación semántica**: El type checker verifica que todos los elementos del literal sean del mismo tipo (o compatibles). El tipo del array se infiere del primer elemento; los demás se comparan contra él. Si hay incompatibilidad, se reporta: "Array element #N has type X, but element #1 has type Y."

#### 7.7.3 Construcción con `new`

La construcción de arrays se realiza con la palabra clave `new`:

```
let numbers = new Number[10];           // Array de 10 números, inicializados a null
let matrix = new Number[3][3];          // Array bidimensional 3x3
let squares = new Number[5] { i -> i * i };  // Array con inicialización lambda
let nested = new Number[][3];           // Array de 3 arrays de números
```

**Variantes soportadas**:

| Sintaxis | `sizes` | `element_type_dims` | `initializer` | Descripción |
|----------|---------|---------------------|---------------|-------------|
| `new T[n]` | `[n]` | 0 | `None` | Array de n elementos, sin inicialización |
| `new T[n]{ i -> expr }` | `[n]` | 0 | `Some(lambda)` | Array de n elementos con lambda |
| `new T[][n]` | `[n]` | 1 | `None` | Array de n arrays de T |
| `new T[n1][n2]` | `[n1, n2]` | 0 | `None` | Array bidimensional |

**Inicialización lambda**: La expresión `{ i -> expr }` evalúa `expr` para cada índice `i` de 0 a n-1. El parámetro `i` es siempre de tipo `Number`. El tipo del body debe ser compatible con el tipo del elemento:

```
let doubles = new Number[5] { i -> i * 2.0 };    // OK
let strings = new String[3] { i -> "item" };     // OK
let bad = new Number[3] { i -> "hello" };         // Error: String vs Number
```

**Verificación semántica del constructor**: El type checker verifica que cada expresión de tamaño sea `Number`, que el tipo base sea válido, y que el body de la lambda sea compatible con el tipo del elemento.

#### 7.7.4 Acceso y asignación de elementos

El acceso a elementos se realiza con corchetes:
...
La asignación a elementos usa el operador `:=`:

```
arr[0] := 100;           // Modifica el primer elemento
arr[1] := arr[0] + 5;    // Lee y modifica
```

**Encadenamiento**: Se puede acceder a elementos de arrays anidados:

```
let matrix = new Number[3][3];
matrix[0][1] := 42;      // Acceso bidimensional
let value = matrix[1][2]; // Lectura bidimensional
```

**Verificación semántica**: El type checker verifica que:
- El índice sea de tipo `Number`.
- El objeto sobre el que se indexa sea de tipo `Array(T)`.
- Para asignación: el tipo del valor sea compatible con el tipo del elemento `T`.
- El resultado de `arr[i]` tiene tipo `T` (el tipo del elemento del array).

#### 7.7.5 Método `.size()`

Cada array tiene un método `.size()` que retorna el número de elementos:

```
let arr = {1, 2, 3};
print(arr.size());  // 3

let matrix = new Number[5][10];
print(matrix.size());  // 5 (primer nivel)
```

**Implementación**: El método `.size()` está implementado como caso especial en el semantic checker y en el codegen, no como un método registrado en el tipo `Array`. Esto significa que no se puede sobrescribir ni invocar mediante dispatch dinámico. En LLVM, se lee directamente el campo `length` del header del array (offset 8).

#### 7.7.6 Layout en memoria

Los arrays usan un layout de **header + buffer de datos** en el heap:

```
+--------+--------+--------+--------+
| offset 0| offset 8| offset 16| offset 24|
| type_id | length  | data_ptr | cursor  |
| (i64)   | (i64)   | (i8*)   | (i64)   |
+--------+--------+--------+--------+
```
...
|--------|-------|--------|-------------|
| 0 | `type_id` | 8 bytes | TypeId del `Array` (para dispatch/`is`) |
| 8 | `length` | 8 bytes | Número de elementos |
| 16 | `data_ptr` | 8 bytes | Puntero al buffer de datos |
| 24 | `cursor` | 8 bytes | Cursor para iteración (inicializado a -1) |

**Buffer de datos**: Cada elemento ocupa exactamente **8 bytes**, independientemente de su tipo real:

| Tipo del elemento | Almacenamiento | Notas |
|-------------------|----------------|-------|
| `Number` (Double) | `double` | 8 bytes directos |
| `Boolean` | `i64` | `zext i1 to i64` antes de store, `trunc` en load |
| `String`, `Null`, `Struct`, `Array` | `i8*` | Puntero genérico |

**Asignación**: Se usa `malloc` para el header (32 bytes) y `malloc` separado para el buffer (`length * 8` bytes). No hay garbage collector; la memoria nunca se libera automáticamente.

**¿Por qué 8 bytes por elemento?** Por simplicidad. Todos los tipos caben en 8 bytes (los punteros son de 64 bits). Esto desperdicia memoria para `Boolean` (1 byte real, 8 bytes asignados), pero simplifica el cálculo de offsets y la generación de código.

#### 7.7.7 Bounds checking en runtime

El compilador genera código de verificación de límites en tiempo de ejecución para cada acceso a elemento (`arr[i]`):

```
; Pseudocódigo LLVM IR
if (index < 0 || index >= length) {
    printf("Array index out of bounds\n");
    // Continúa con ejecución (potencialmente unsafe)
}
```

**Comportamiento actual**: Cuando el índice está fuera de rango, se imprime un mensaje de error pero la ejecución **continúa** con el valor que haya en esa posición de memoria. Esto es un bug conocido — la ejecución debería detenerse.

**¿Por qué no compile-time bounds checking?** Porque los índices generalmente no son conocidos en compilación. `arr[i]` donde `i` es una variable no puede verificarse estáticamente.

#### 7.7.8 Limitaciones de los arrays

1. **No se pueden imprimir**: `print(arr)` produce error de compilación: "Function 'print' cannot print values of type Array."
2. **Literal vacío no soportado**: `{}` es un bloque vacío, no un array vacío. Para crear un array vacío se debe usar `new T[0]`.
3. **Sin resizing**: No hay métodos `push`, `pop`, `append` ni `insert`. El tamaño se fija en la construcción.
4. **Sin bounds check en asignación**: `arr[i] := v` no verifica que `i` esté dentro de rango. Solo la lectura `arr[i]` tiene bounds check (y no aborta).
5. **Memory leak**: Los arrays se asignan con `malloc` y nunca se liberan.
6. **Inicialización a null**: `new Number[3]` inicializa los elementos a `null` (no a `0.0`), lo cual es inseguro si se leen antes de escribir.
7. **Cursor no utilizado**: El campo `cursor` (offset 24) está reservado pero no se usa en la implementación actual de iteración.
...
**Print de arrays**: No soportado. `print(arr)` produce error de compilación. El programador debe iterar manualmente sobre los elementos.

**Literal vacío de arrays**: `{}` es una expresión de bloque, no un array vacío. Para crear un array vacío se debe usar `new T[0]`, lo cual es menos intuitivo.

**Sin resizing de arrays**: No hay métodos `push`, `pop`, `append` ni `insert`. El tamaño de un array se fija en su construcción y no puede modificarse.

**Bounds check incompleto en arrays**: La lectura `arr[i]` verifica límites en runtime pero no aborta la ejecución (solo imprime un error). La asignación `arr[i] := v` no tiene ningún bounds check.

**Memory leak en arrays**: Los arrays se asignan con `malloc` y nunca se liberan. No hay garbage collector. Esto es parte del problema general de memory leaks descrito más adelante.
...
5. **Arrays y vectores**: Soporte completo para arrays unidimensionales y multidimensionales con acceso por índice, asignación destructiva, construcción con inicialización lambda, método `.size()`, y validación de tipos en el semantic checker.
...
15. **Garbage Collector para arrays**: Los arrays se asignan con `malloc` y nunca se liberan. Un GC o reference counting es esencial para programas que crean muchos arrays temporales.
16. **Print de arrays**: Implementar impresión de arrays para debugging, incluyendo arrays anidados. Podría usar `print_array(arr)` con formato `[1, 2, 3]`.
17. **Literal vacío**: Soportar `new T[]` o `{}` como array vacío sin ambigüedad con expresiones de bloque.
18. **push/pop/append**: Operaciones de resizing dinámico para arrays, similares a `Vec` en Rust o `ArrayList` en Java.
19. **Bounds check que aborte**: Modificar el bounds check en runtime para que termine la ejecución (con `exit(1)`) en lugar de solo imprimir un error y continuar.
20. **Inicialización segura**: `new Number[n]` debería inicializar elementos a `0.0` en lugar de `null` para evitar undefined behavior.
